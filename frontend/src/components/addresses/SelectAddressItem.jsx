...
function AddressIcon({ label, className }) {
   ...
   if (normalized === 'home') return <Home className={className} />;
   if (normalized === 'work') return <BriefcaseBusiness className={className} />;
   ...
   return (
      ...
      >
         <div className="mt-0.5 flex size-8.5 shrink-0 items-center justify-center rounded-lg bg-[#FFF4F5] text-[#E56A77] transition-colors duration-150 group-hover:bg-[#E56A77] group-hover:text-white">
            <AddressIcon label={address.label} className="size-4" />
         </div>
